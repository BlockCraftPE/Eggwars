...
use Driesboy\EggWars\Command\{Hub, EW};
use Driesboy\EggWars\Task\{SignManager, Game};
...
class EggWars extends PluginBase{
    ...
    public function configPrepare(){
        ...
        if(!$cfg->get("StartTime")){
            $cfg->set("StartTime", 61);
        }
        if(!$cfg->get("EndTime")){
            $cfg->set("EndTime", 16);
        }
        if(!$cfg->get("BuildBlocks")){
            $cfg->set("BuildBlocks", array(24, 20, 89, 49, 54, 92, 121));
        }
        $cfg->save();
